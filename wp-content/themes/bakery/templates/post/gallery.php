<?php if ( ! defined( 'ABSPATH' ) ) exit(); ?>

<article id="post-<?php the_ID(); ?>" <?php post_class( 'vu_blog-post vu_bp-type-gallery' ); ?>>
	<?php $bakery_post_format_settings = bakery_get_post_meta( $post->ID, 'bakery_post_format_settings' ); ?>

	<?php if ( ! empty( $bakery_post_format_settings['gallery']['images'] ) ) : ?>
		<div class="vu_bp-image">
			<?php 
				echo do_shortcode( bakery_generate_shortcode( 'vu_image_slider', array(
					'_images' => $bakery_post_format_settings['gallery']['images'],
					'ratio' => bakery_get_option( 'blog-image-ratio', '2:1' ),
					'lightbox' => '1',
					'navigation' => 'hover',
					'pagination' => 'always',
					'color' => '#fffff',
					'autoplay' => '',
					'class' => ''
				) ) );
			?>
		</div>
	<?php elseif ( has_post_thumbnail() ) : ?>
		<?php get_template_part( 'templates/post/meta/image' ); ?>
	<?php endif; ?>
	
	<div class="vu_bp-content-wrapper">
		<header class="vu_bp-header">
			<?php get_template_part( 'templates/post/parts/title' ); ?>
			
			<div class="vu_bp-meta">
				<?php get_template_part( 'templates/post/meta/date' ); ?>

				<?php get_template_part( 'templates/post/meta/author' ); ?>

				<?php get_template_part( 'templates/post/meta/categories' ); ?>

				<?php get_template_part( 'templates/post/meta/comments' ); ?>

				<?php get_template_part( 'templates/post/meta/tags' ); ?>

				<div class="clear"></div>
			</div>					
			<div class="clear"></div>
		</header>

		<div class="vu_bp-content">
			<?php get_template_part( 'templates/post/parts/content' ); ?>
		</div>
	</div>
	<div class="clear"></div>
</article>
