...
// Variables
$custom_class = 'vu_s-custom-'. rand(100000, 999999);
$custom_css = '';
...
if ( preg_match('~\bbackground(-image)?\s*:(.*?)\(\s*(\'|")?(?<image>.*?)\3?\s*\)~i', $atts['css'], $match) ) {
	...
	if ( !empty($atts['bg_image']) ) {
		...
		$custom_css .= '.vu_section .'. esc_attr($custom_class) .'{background-image:url('. esc_url($atts['bg_image']) .')}';
	}
}

// Image background
if ( $atts['bg_type'] == 'image' and !empty($atts['bg_image']) ) {
	$atts['class'] .= ' vu_s-bg-image';
}

// Video background
if ( $atts['bg_type'] == 'video' and !empty($atts['bg_video']) ) {
	$atts['class'] .= ' vu_s-bg-video';
...
if ( !empty($atts['color_overlay']) ) {
	$atts['class'] .= ' vu_s-color-overlay';
	$custom_css .= '.vu_section .'. esc_attr($custom_class) .'.vu_s-wrapper.vu_s-color-overlay:before{background-color:'. esc_attr($atts['color_overlay']) .'}';
}

// Pattern overlay css class
if ( $atts['enable_pattern'] == 'true' && !empty($atts['pattern_image']) ) {
	$atts['class'] .= ' vu_s-pattern-overlay';
	$custom_css .= '.vu_section .'. esc_attr($custom_class) .'.vu_s-wrapper.vu_s-pattern-overlay:after{background-image:url('. esc_url(bakery_get_attachment_image_src($atts['pattern_image'], 'full')) .');'. (!empty($atts['pattern_opacity']) ? 'opacity:'. esc_attr($atts['pattern_opacity']) : '') .'}';
}
